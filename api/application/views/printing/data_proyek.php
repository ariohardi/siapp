...
<table class="zui-table" <?php if($this->input->get('export')==="excel") echo "border='1'" ?>>
    ...
    <tbody>
        <?php foreach ($data as $key => $value) {
        	...
        	<td align='center'>{$value->nama_proyek}</td>
            <td align='center'>{$value->nama_klien}</td>
            <td align='center'>{$value->nama_propinsi}</td>
            <td align='center'>{$value->nama_kabupaten}</td>
            <td align='center'>{$value->chief_security}</td>
        	</tr>";
        } ?>
    </tbody>
</table>
