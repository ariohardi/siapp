angular.module("RDash").controller("Administrator_baru",function($scope,$rootScope,$http,$location,$timeout){
	$scope.loading = true;
	$scope.FormData = {};
	$scope.FormData.kabupaten = '';
	$rootScope.title_tag = "Buat Admin Baru";
	...
	$scope.submit = function(){
		if($scope.FormData.p1.length<6){
			jQuery.toast({
				text : "Panjang password minimal 6 karakter",
				icon : "error"
			});
		} else if($scope.FormData.p1!==$scope.FormData.p2){
			jQuery.toast({
			...
			$scope.loading = true;
			$http.post('api/index.php/admin/simpan',$scope.FormData).success(function(data){
				$scope.loading = false;
				$.toast({
